# EssayWriter Orchestrator (Pipeline Mode) Architecture

> 🚧 Under development. This document describes the Web App / Orchestrator
> ("Pipeline Mode") path, in which the FastAPI backend owns workflow state and
> makes its own LLM API calls. For the recommended way to run EssayWriter today,
> see the Dynamic Workflows section of the [README](../README.md).

This is the deep architecture reference that previously lived in the README: the
stage-by-stage pipeline, the human-in-the-loop gates, per-step LLM usage, the
prompt inventory, and orchestrator configuration.
